...
def menu_log():
    ...
    print('%s║'%(O))
    if pmu in ['']:
        jalan('%s╚══[%s!%s] %sFILL IN CORRECTLY'%(M,P,M,P))
        menu_log()
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sFor Example :  srilanka,srilanka123,123456,786786'%(O,P,O,P))
    cuy = input('%s╠══[%s•%s] %sEnter Manual Additional Pass [1 Word] : '%(O,P,O,P))
    gh = open('pass.txt','w')
    gh.write(cuy)
    gh.close
...
def target():
    ...
    jalan('%s╠══[%s•%s] %sBIRTHDAY       : %s'%(O,P,O,P,ut))
    ...
    jalan('%s╠══[%s•%s] %sMERETIAL STATUS: %s'%(O,P,O,P,rl))
    jalan('%s╠══[%s•%s] %sRELARION WITH  : %s'%(O,P,O,P,rls))
    jalan('%s╠══[%s•%s] %sCURRENT CITY   : %s'%(O,P,O,P,lc))
    jalan('%s╠══[%s•%s] %sHOMETOWN       : %s'%(O,P,O,P,ht))
...
def var_author():
    ...
    mlaku('%s║     • %sUJ'%(O,P))
    mlaku('%s║     • %sUNIC TORBY'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sJayasingha %sHUS%s :'%(O,P,O,P,O,P))
    mlaku('%s║'%(O))
def var_ugen():
